...
#define KR_PS2_DRIVER    KR_APP(1) /* The key for PS/2 driver */
#define KR_EVENT_MGR     KR_APP(2) /* The key for event mgr */
#define KR_WINDOW_SYS    KR_APP(3) /* The key for window system */
#define KR_PCI_PROBE     KR_APP(4) /* The key for the PCI prober */

#define KR_SESSION_CREATOR         KR_APP(5)
#define KR_TRUSTED_SESSION_CREATOR KR_APP(6)
#define KR_SESSION                 KR_APP(7)

#define KR_ETERM_C      KR_APP(8) 
#define KR_ETERM        KR_APP(9) 
#define KR_COPIER       KR_APP(10)
#define KR_PASTER       KR_APP(11)
...
main(void)
{
  ...
  node_extended_copy(KR_CONSTIT, KC_ETERM_C, KR_ETERM_C);
  node_extended_copy(KR_CONSTIT, KC_COPIER, KR_COPIER);
  node_extended_copy(KR_CONSTIT, KC_PASTER, KR_PASTER);
  ...
  /* Construct one ETERM domain for the copier */
  if (constructor_request(KR_ETERM_C, KR_BANK, KR_SCHED, KR_SESSION_CREATOR,
			  KR_ETERM) != RC_OK) {
    kprintf(KR_OSTREAM, "** ERROR: couldn't construct eterm.\n");
    return -1;   /* FIX:  really need to terminate gracefully */
  }

  kprintf(KR_OSTREAM, "Master: now constructing copier...\n");
  if (constructor_request(KR_COPIER, KR_BANK, KR_SCHED, KR_ETERM,
			  KR_VOID) != RC_OK) {
    kprintf(KR_OSTREAM, "** ERROR: couldn't construct copier domain.\n");
    return -1;   /* FIX:  really need to terminate gracefully */
  }

  /* Construct another ETERM domain for the paster */
  if (constructor_request(KR_ETERM_C, KR_BANK, KR_SCHED, KR_SESSION_CREATOR,
			  KR_ETERM) != RC_OK) {
    kprintf(KR_OSTREAM, "** ERROR: couldn't construct eterm.\n");
    return -1;   /* FIX:  really need to terminate gracefully */
  ...
  kprintf(KR_OSTREAM, "Master: now constructing paster...\n");
  if (constructor_request(KR_PASTER, KR_BANK, KR_SCHED, KR_ETERM,
			  KR_VOID) != RC_OK) {
    kprintf(KR_OSTREAM, "** ERROR: couldn't construct paster domain.\n");
    return -1;   /* FIX:  really need to terminate gracefully */
  }
