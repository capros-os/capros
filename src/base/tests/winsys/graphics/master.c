...
#define KR_CLIENT        KR_APP(7) /* The key for the client
				      constructor */
...
main(void)
{
  ...
  node_extended_copy(KR_CONSTIT, KC_CLIENT_C, KR_CLIENT);
  ...
  if (eros_domain_winsys_master_get_session_creators(KR_WINDOW_SYS, 
    ...
    /* Now construct the sample client, passing it the key to an
       appropriate session creator. */
  if (constructor_request(KR_CLIENT, KR_BANK, KR_SCHED, KR_SESSION_CREATOR,
			  KR_CLIENT) != RC_OK) {
    kprintf(KR_OSTREAM, "** ERROR: couldn't construct sample client domain\n");
    return -1;   /* FIX:  really need to terminate gracefully */
  }
