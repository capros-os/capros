...
/* This test demonstrates how subsessions and invisible windows work.
It creates a top-level window, then an invisible window within that
top-level. Finally, it creates a subsession with the invisible window
as its container and hands that to another domain. */
...
#define KR_SUBSESSION_CREATOR KR_APP(16)
#define KR_SUB_DOMAIN         KR_APP(17)
...
main(void)
{
  ...
  node_extended_copy(KR_CONSTIT, KC_SUB_C, KR_SUB_DOMAIN);
  ...
  if (probe_and_init_video(KR_WINDOW_SYS, KR_PCI_PROBE) == RC_OK) {
	...
	return -1;
      ...
      win_clear(0, parent_size, COLOR3);

      /* Create invisible window that is child of the main window */
      result = session_new_invisible_window(KR_SESSION,
					    KR_SUB_BANK,
					    parent,
					    child_orig.x,
					    child_orig.y,
					    child_size.x,
					    child_size.y,
					    0x0u, /* normal invisible window */
					    &invisible);


      if (result != RC_OK) {
	kprintf(KR_OSTREAM, "Error: couldn't create invisible window; "
		"ret=0x%08x", result);
	return -1;
      }

      /* Tell the window system that we want to display the top-level
	 window */
      session_win_map(KR_SESSION, parent);

      /* Even though we have an invisible child, if we don't map it
	 none of its children will be visible. (Go ahead... try it.)
	 */
      session_win_map(KR_SESSION, invisible);

      /* Create a subsession with the invisible window as container */
      session_new_session_creator(KR_SESSION, KR_BANK, invisible, 
				  KR_SUBSESSION_CREATOR);

      /* Construct an external domain and pass it the subsession
	 creator key. */
      kprintf(KR_OSTREAM, "About to construct domain for sub-session...");
      if (constructor_request(KR_SUB_DOMAIN, KR_BANK, KR_SCHED, 
			      KR_SUBSESSION_CREATOR,
			      KR_VOID) != RC_OK) {
	kprintf(KR_OSTREAM, 
		"** ERROR: couldn't construct sub-session domain...\n");
	return -1;   /* FIX:  really need to terminate gracefully */
      }
	...
	if (result != RC_OK)
	  ...
	  if (IS_RESIZE_EVENT(evt)) {
	    ...
	    else if (evt.window_id == invisible) {
	      kprintf(KR_OSTREAM, 
		      "TEST: Error: rec'd resize event for INVISIBLE...\n");
	    }
	    else 
