...
uint32_t
drawable_trifill(uint32_t drawableKey, uint32_t pt1x, uint32_t pt1y, 
		  uint32_t pt2x, uint32_t pt2y, uint32_t pt3x, 
		  uint32_t pt3y, uint32_t color, uint32_t raster_op)
{
  Message m;
  uint32_t send_data[8];

  m.snd_invKey = drawableKey;
  m.snd_code = OC_Drawable_TriFill;
  m.snd_key0 = KR_VOID;
  m.snd_key1 = KR_VOID;
  m.snd_key2 = KR_VOID;
