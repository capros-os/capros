                  ...
                  uint32_t krTo0, uint32_t krTo1, uint32_t krTo2)
{
  switch (count) {
  case 1:
    return eros_SpaceBank_alloc1(krBank, eros_Range_otNode, krTo0);
  case 2:
    return eros_SpaceBank_alloc2(krBank,
      eros_Range_otNode | (eros_Range_otNode << 8),
      krTo0, krTo1);
  case 3:
    return eros_SpaceBank_alloc3(krBank,
      eros_Range_otNode | ((eros_Range_otNode | (eros_Range_otNode << 8)) << 8),
      krTo0, krTo1, krTo2);
  default:
