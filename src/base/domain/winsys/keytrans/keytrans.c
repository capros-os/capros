...
#define ASCII_DOWN    0
#define ASCII_UP      1
#define SPECIAL_DOWN  2
#define SPECIAL_UP    3
#define LOCKING_DOWN  4
#define LOCKING_UP    5

/* globals for scancode conversion */
static int state = 0;
...
/* Needed for scancode conversion */
int
...
/* Scancode conversion */  
bool
convert_scancode(int32_t data, uint8_t *c, bool *special)
{
  if (c == NULL)
    return false;
  ...
  if (data > -1) { 
    ...
    if ((updShiftState(scode, data & 0x80, &state)) == ASCII_DOWN) {
      i = getstate(scode, &state);
      *c = key_map.key[scode].map[i];
      *special = false;
      return true;
    }
    else if ((updShiftState(scode, data & 0x80, &state)) == SPECIAL_DOWN) {
      i = getstate(scode, &state);
      *c = key_map.key[scode].map[i];
      *special = true;
      return true;
