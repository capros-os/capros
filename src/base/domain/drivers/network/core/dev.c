...
{
	unsigned hash = full_name_hash((const unsigned char *)name,
				 strnlen(name, IFNAMSIZ));
...
struct net_device *dev_getbyhwaddr(unsigned short type, u8 * ha)
{
	struct net_device *dev;
...
int dev_alloc_name(struct net_device *dev, const char *name)
{
	...
	char buf[IFNAMSIZ];
	const char *p;
	const int max_netdevices = 32*32;	// was 8*PAGE_SIZE
	struct net_device *d;
	...
	if (p) {
		...
		unsigned long inuse[max_netdevices/32];
