...
#define KR_TG3       KR_APP(3)

#define DEBUG_HELPER if(0)
...
main(void)
{
  ...
  DEBUG_HELPER kprintf(KR_OSTREAM, "helper:irq to wait on = %d",IRQ);
  ...
  for(;;) {
    ...
    if (result != RC_OK) {
      DEBUG_HELPER
	kprintf(KR_OSTREAM, "TG3 helper: ERROR on IRQ LINE %d",IRQ);
    }else {  
      DEBUG_HELPER kprintf(KR_OSTREAM,"Received IRQ on %d",IRQ);
