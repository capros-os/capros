...
/* This process is a helper to ps2reader. It waits for IRQ1 to arrive
 * and then turns around and calls ps2reader signalling the arrival of
 * IRQ1, so that ps2reader can read the ps2 h/w buffer for keyboard data */
...
main(void)
{
  ...
  /* Loop infinitely waiting for IRQ1 */
  for(;;) {
    result = capros_DevPrivs_waitIRQ(KR_DEVPRIVS, 1);
    if (result != RC_OK) {
      kprintf(KR_OSTREAM, "helper1: ERROR on 1");
    }else {  
      /* We have an IRQ on 1 !! Signal ps2reader */
      (void)capros_Ps2_irqArrived(KR_PS2READER,capros_Ps2_IRQ1);
    }
  }
  return 0;
}
