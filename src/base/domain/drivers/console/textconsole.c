...
main(void) 
{
  ...
  msg.rcv_data = &chArrayBuf;
  msg.rcv_limit = sizeof(chArrayBuf);
  msg.rcv_code = 0;
  msg.rcv_w1 = 0;
  msg.rcv_w2 = 0;
  msg.rcv_w3 = 0;
...
initialize(void)
{
  ...
  result = capros_GPT_setL2v(KR_ADDRSPC, BlssToL2v(EROS_PAGE_BLSS + 2));
  ...
  if(result!=RC_OK) kprintf(KR_OSTREAM,"textconsole::make noCall--Failed");
...
#error "This is an i386 console driver.  You are not running an i386.  Sorry."
  ...
  if (result != RC_OK || currentType != capros_Range_otNone)
    kprintf(KR_OSTREAM, "textconsole::waitCap--Failed");
...
ProcessRequest(Message *msg) 
{
  ...
  case OC_console_textconsole_clearScreen:
    {
      clearScreen();
      msg->snd_code = RC_OK;
      return 1;
  ...
  case OC_console_textconsole_putCharArray:
    {
      int i;
      uint32_t len;
      uint8_t ch;
      ...
      kprintf(KR_OSTREAM,"putchararray len,max = %d,%d",chArrayBuf.len,chArrayBuf.max);

      for (i = 0; i < chArrayBuf.len; i++) {
	ch = chArrayBuf.data[i];
	kprintf(KR_OSTREAM,"ch[%d] = %d",i,chArrayBuf.data[i]);
	processInput(ch);
      }
      msg->snd_code = RC_OK;
      msg->snd_w1 = len; /* return the number of characters processed */
      return 1;
    }
  case OC_console_textconsole_putChar:
    {
      uint8_t ch;
