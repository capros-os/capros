...
find_arp_entry(void)
{
  ...
  for (i = 0; i < ARP_TABLE_SIZE; ++i) {
    if (arp_table[i].state == ETHARP_STATE_EMPTY) {
      DEBUGF kprintf(KR_OSTREAM,"find_arp_entry: found empty entry %d", i);
      break;
    }
  }
		 ...
		 struct eth_addr *ethaddr, uint8_t flags)
{
  ...
  DEBUGF kprintf(KR_OSTREAM,"update_arp_entry:IP address not yet in table");
  /* allowed to insert an entry? */
  if ((ETHARP_ALWAYS_INSERT) || (flags & ARP_INSERT_FLAG)) {
    DEBUGF kprintf(KR_OSTREAM,"update_arp_entry:adding entry to table");
    /* find an empty or old entry. */
    i = find_arp_entry();
    if (i == ARP_TABLE_SIZE) {
      DEBUGF kprintf(KR_OSTREAM,"update_arp_entry:no available entry found");
      return NULL;
    }
...
etharp_ip_input(struct netif *netif, struct pbuf *p)
{
  ...
  DEBUGF kprintf(KR_OSTREAM,"etharp_ip_input: updating ETHARP table");
...
etharp_arp_input(struct netif *netif, struct eth_addr *ethaddr, struct pbuf *p)
{
  ...
  case ARP_REQUEST:
	...
	kprintf(KR_OSTREAM,
      ...
      /* return ARP reply */
      netif_start_xmit(p,-1,&(hdr->dipaddr));
    } else {
      DEBUGF
      kprintf(KR_OSTREAM,"etharp_arp_input:incoming ARP req. was not for us");
    }
    break;
  case ARP_REPLY:    
...
etharp_query(struct netif *netif, struct ip_addr *ipaddr, struct pbuf *q)
{
  ...
  if (perform_arp_request) {
	...
	hdr->ethhdr.src.addr[j] = srcaddr->addr[j];
      ...
      hdr->ethhdr.type = htons(ETHTYPE_ARP);      
      /* send ARP query */
      result = netif_start_xmit(p,-1,(struct ip_addr*)IP_ADDR_BROADCAST);
      /* free ARP query packet */
      pbuf_free(p);
      p = NULL;
    } else {
      result = RC_ERR_MEM;
      kprintf(KR_OSTREAM,"etharp_query:couldn't alloc pbuf for ARP request.");
    }
  }
  return result;
}
...
struct eth_addr *
etharp_lookup(struct netif *netif,struct ip_addr *ipaddr,struct pbuf *p)
{
  int i;
