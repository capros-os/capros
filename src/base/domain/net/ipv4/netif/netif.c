...
/* The generic network card probe function.*/
uint32_t 
netif_probe()
{
#if NETWORK_CARD==LANCE
  return lance_probe();
#elif NETWORK_CARD==TORNADO
  return _3c905c_probe();
#elif NETWORK_CARD==ALTIMA
  return altima_probe();
#else 
  return 1;
#endif
...
/* The generic network ISR */
uint32_t 
netif_interrupt()
{
#if NETWORK_CARD==LANCE
  return lance_interrupt();
#elif NETWORK_CARD==TORNADO
  return boomerang_interrupt();
#elif  NETWORK_CARD==ALTIMA
  return tg3_interrupt();
#else
  return 1;
#endif
}
...
/* The generic network ISR */
uint32_t 
netif_start_xmit(struct pbuf *p,int type,struct ip_addr *ip)
{
#if NETWORK_CARD==LANCE
  return lance_start_xmit(p,type,ip);
#elif NETWORK_CARD==TORNADO
  return boomerang_start_xmit(p,type,ip);
#elif NETWORK_CARD==ALTIMA
  return tg3_start_xmit(p,type,ip);
#else 
    return 1;
#endif
}
