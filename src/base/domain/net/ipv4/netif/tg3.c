...
#define KR_DMA KR_TIMEOUT_AGENT
 ...
 * address space */
#define LWK_FACTOR(lss) (mult_tg3(EROS_NODE_SIZE, lss) * EROS_PAGE_SIZE)
uint32_t 
mult_tg3(uint32_t base, uint32_t exponent)
{
  uint32_t u;
  int32_t result = 1u;
...
patch_addrspace_tg3()
{
  ...
  for (next_slot = 1; next_slot < 16; next_slot++) {
    ...
    window_key.value[1] = 0;    /* high order 32 bits of address
                                   offset */
    /* low order 32 bits: mult_tg3iple of EROS_NODE_SIZE ^ (LSS-1) pages */
 ...
 *  correct page keys. */
void
init_mapped_memory_tg3(uint32_t *base, uint32_t size)
{
  uint32_t u;
		 ...
		 int src_idx, uint32_t dest_idx_unmasked)
{
  ...
  mapping += skb_size;
  if(mapping > 0x30000u) mapping = 0x20000u;
  return skb_size;
}
...
tg3_rx(struct tg3 *tp)
{
  ...
  while (sw_idx != hw_idx) {
    ...
    base = bus_to_virt(desc->addr_lo);
    
    tg3_alloc_rx_skb(tp, opaque_key,desc_idx, *post_ptr);

    DEBUG_ALTIMA kprintf(KR_OSTREAM,"Len = %d base = %08x::%08x",len,
			 desc->addr_hi,bus_to_virt(desc->addr_lo));
    
    /* The packet receiving block. Allocate a pbuf, dump the packets
     * contents onto it and send it to our layer above*/
    {
      struct pbuf *p,*q;
      uint16_t totlen = 0; 
      char *s;
      ...
      if(p!=NULL) {
	DEBUG_ALTIMA kprintf(KR_OSTREAM,"POOL Allocated ");
	...
	case ETHERTYPE_IP:
	  ...
	  DEBUG_ALTIMA kprintf(KR_OSTREAM,"tg3_rx ip pkt");
	  pbuf_header(p,-14);
	  ip_input(p);
	  break;
	case ETHERTYPE_ARP:
	  DEBUG_ALTIMA kprintf(KR_OSTREAM,"etharp packet");
	  ...
	  break;
	default:
	  DEBUG_ALTIMA kprintf(KR_OSTREAM,"tg3_rx default pkt");
	...
	kprintf(KR_OSTREAM,"tg3_rx() pbuf exhausted");
      }
    }
...
unsigned int tg3_has_work(struct tg3 *tp)
{
  ...
  if (!(tp->tg3_flags &
    ...
    if (sblk->status & SD_STATUS_LINK_CHG) {
      DEBUG_ALTIMA kprintf(KR_OSTREAM,"Link XCHG");
      tg3_setup_phy(tp);
      work_exists = 1;
    }
  }
...
uint32_t
tg3_start_xmit(struct pbuf *p,int type,struct ip_addr *ipaddr)
{
  uint32_t mapping = 0x17000u;
  ...
  int i;
  unsigned char *tbuf = (void *)(DMA_START + mapping);
  ...
  if(type != -1) {
    ...
    for(i=0;i<ETHER_ADDR_LEN;i++) {
      tbuf[i] = dest->addr[i];
      tbuf[i+ETHER_ADDR_LEN] = NETIF.hwaddr[i];
    }
    tbuf[ETH_ADDR_LEN*2] = type >> 8;
    tbuf[ETH_ADDR_LEN*2+1] = type;
    totlen = ETHER_HDR_LEN;
  }
  
  for(q = p; q != NULL; q = q->next) {
    for(i=0;i<q->len;i++) {
      ((char *)tbuf)[totlen++] = ((char *)q->payload)[i];
    }
  }
  ...
  tg3_set_txd(tp, entry, PHYSADDR + mapping,totlen, base_flags,1);
...
altima_probe()
{
  ...
  patch_addrspace_tg3();
  init_mapped_memory_tg3(dma_abszero,DMA_SIZE);
  ...
  if((result=StartHelper_tg3())!= RC_OK) {
  ...
  /* Abbrev for lance */
  NETIF.name[0] = 't'; NETIF.name[1] = 'g'; NETIF.name[2] = '3';
  
  /* Get the MAC Address */
  NETIF.hwaddr_len = 6; /* ethernet has 6 byte long h/w length */
  for(i = 0; i < 6; i++)  NETIF.hwaddr[i] = eaddr[i];

  /* Set the MTU */
  NETIF.mtu = tp->mtu; /* Maximum MTU on ethernet */

  NETIF.ip_addr.addr = 0x0201010A;
  NETIF.netmask.addr = 0x00FFFFFF;
  NETIF.gw.addr = 0x0101010A;
  return RC_OK;
}
