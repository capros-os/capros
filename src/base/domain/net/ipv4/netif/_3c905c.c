...
uint32_t
boomerang_start_xmit(struct pbuf *p,int type,struct ip_addr *ipaddr)
{
  long ioaddr = NETDEV.base_address[0];
  int totlen = 0,i;
  struct pbuf *q;
  ...
  if(type != -1) {
    ...
    for(i=0;i<ETHER_ADDR_LEN;i++) {
      vp->tbuf[i] = dest->addr[i];
      vp->tbuf[i+ETHER_ADDR_LEN] = NETIF.hwaddr[i];
    }
  ...
  for(q = p; q != NULL; q = q->next) {
    for(i=0;i<q->len;i++) {
      ((char *)vp->tbuf)[totlen++] = ((char *)q->payload)[i];
    }
  }
...
boomerang_rx()
{
  ...
  while ((rx_status = le32_to_cpu(vp->rx_ring[entry].status)) & RxDComplete){
	...
	kprintf(KR_OSTREAM,"Receiving packet size %d status %x at dma = %x",
      ...
      if(p!=NULL) {
	DEBUG_3C905C kprintf(KR_OSTREAM,"POOL Allocated ");
	s = (char *)bus_to_virt(vp->rx_ring[entry].addr);
	...
	case ETHERTYPE_IP:
	  ...
	  DEBUG_3C905C kprintf(KR_OSTREAM,"boomerang_rx ip pkt");
	  pbuf_header(p,-14);
	  ip_input(p);
	  break;
	case ETHERTYPE_ARP:
	  ...
	  break;
	default:
	  kprintf(KR_OSTREAM,"boomerang_rx default pkt");
...
_3c905c_probe()
{
  ...
  result = constructor_request(KR_MEMMAP_C,KR_BANK,KR_SCHED,
			       KR_VOID,KR_MEMMAP_C);
  if(result!=RC_OK) {
