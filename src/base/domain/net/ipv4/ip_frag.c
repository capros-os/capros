...
static struct pbuf *
copy_from_pbuf(struct pbuf *p, uint16_t * offset,
				   uint8_t * buffer, uint16_t len)
{
  uint16_t l;
  
  p->payload = (uint8_t *)p->payload + *offset;
  p->len -= *offset;
  while (len) {
...
ip_frag(struct pbuf *p)
{
  ...
  while (left) {
    ...
    p = copy_from_pbuf(p, &poff, (uint8_t *) iphdr + IP_HLEN, cop);
    ...
    netif_start_xmit(header,ETHERTYPE_IP,&(iphdr->dest));
    pbuf_free(header);
