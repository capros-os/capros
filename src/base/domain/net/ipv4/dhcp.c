...
void dhcp_recv(void *arg, struct udp_pcb *pcb, 
		      struct pbuf *p, struct ip_addr *addr, uint16_t port);
		      ...
		      struct ip_addr *addr, uint16_t port)
{
  ...
  for (i = 0; i < netif->hwaddr_len; i++) {
    if (netif->hwaddr[i] != reply_msg->chaddr[i]) { 
      DEBUGDHCP
      kprintf(KR_OSTREAM,"netif->hwaddr[%u]=%2x != reply_msg->chaddr[%u]=%2x",
...
uint32_t dhcp_create_request(struct netif *netif)
{
  ...
  if (dhcp->p_out == NULL) {
    kprintf(KR_OSTREAM,"dhcp_create_request(): could not allocate pbuf");
    return RC_NetSys_PbufsExhausted;
  }
