             ...
             uint8_t proto)
{
  ...
  if (NETIF.mtu && (p->tot_len > NETIF.mtu)) {
    kprintf(KR_OSTREAM,"Fragmenting the packet tot_len = %d NETIF.mtu = %d",
	    p->tot_len, NETIF.mtu);
    return ip_frag(p);
  }
  
  return netif_start_xmit(p,ETHERTYPE_IP,&(iphdr->dest));  
}
...
ip_input(struct pbuf *p) {
  ...
  if(iphdrlen > p->len) {
    kprintf(KR_OSTREAM,"hdr len > first pbuf length");
    pbuf_free(p);
    return RC_OK;
  }
  ...
  if((IPH_OFFSET(iphdr) & htons(IP_OFFMASK | IP_MF)) != 0) {
    kprintf(KR_OSTREAM,"Reassembling the packet");
    p = ip_reass(p);
  ...
  case IP_PROTO_ICMP: 
    {
      ...
      * and never come in here */
      if(!ip_addr_isany(&NETIF.ip_addr)) icmp_input(p,&NETIF);
      break;
    }
  default:
