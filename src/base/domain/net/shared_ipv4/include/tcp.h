...
void             tcp_input   (struct pstore *p , int ssid,struct netif *inp);
...
struct tcp_seg {
  ...
  struct pstore *p;        /* buffer containing data + TCP header */
  void *dataptr;           /* pointer to the TCP data in the pstore */
...
uint8_t tcp_segs_free(struct tcp_seg *seg,int ssid);
uint8_t tcp_seg_free(struct tcp_seg *seg,int ssid);
struct tcp_seg *tcp_seg_copy(struct tcp_seg *seg,int ssid);
