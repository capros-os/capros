...
struct udp_pcb {
  ...
  void (* recv)(void *arg, struct udp_pcb *pcb, struct pstore *p,
  ...
  /* The application process is given this pstore when udp data is received */
  struct pstore *pstore;
...
uint32_t udp_send(struct udp_pcb *pcb,struct pstore *p);
...
void udp_recv(struct udp_pcb *pcb,
	      void (* recv)(void *arg, struct udp_pcb *upcb,
			    struct pstore *p, struct ip_addr *addr,
			    uint16_t port),void *recv_arg);
void  udp_input(struct pstore *p,int ssid);
