...
static struct pstore *recv_data;
 ...
 */
void
tcp_input(struct pstore *p, int ssid, struct netif *inp)
{
  ...
  tcphdr = (struct tcp_hdr *)((uint8_t *)PSTORE_PAYLOAD(p,ssid)
			      + IPH_HL(iphdr) * 4);

  DEBUG_TCPIN kprintf(KR_OSTREAM,"Into tcp_in");
  /* remove header from payload */ 
  if (pstore_header(p, -((int16_t)(IPH_HL(iphdr) * 4)),ssid) 
...
#if 0
  /* Verify TCP checksum. */
  if (inet_chksum_pseudo(p,ssid,(struct ip_addr *)&(iphdr->src),
  ...
  /* Move the payload pointer in the pstore so that it points to the
   *  TCP data instead of the TCP header. */
  offset = TCPH_OFFSET(tcphdr) >> 4;
  pstore_header(p, -(offset * 4),ssid);
  ...
  if (pcb != NULL) {
    ...
    inseg.next = NULL;
    inseg.len = p->tot_len;
    inseg.dataptr = PSTORE_PAYLOAD(p,ssid);
    inseg.p = p;
    inseg.tcphdr = tcphdr;
    ...
    /* We deallocate the incoming pstore. If it was buffered by the
     * application, the application should have called pstore_ref() to
     * increase the reference counter in the pstore. If so, the buffer
     * isn't actually deallocated by the call to pstore_free(), only the
     * reference count is decreased. */
    pstore_free(inseg.p,pcb->ssid);
#if TCP_INPUT_DEBUG
#if TCP_DEBUG
    tcp_debug_print_state(pcb->state);
#endif /* TCP_DEBUG */
...
tcp_listen_input(struct tcp_pcb_listen *pcb)
{
  ...
  if (flags & TCP_ACK) {
    ...
    npcb->ssid = pcb->ssid; /* Keep note of our ssid */
    
    DEBUG_TCPIN
      kprintf(KR_OSTREAM,"npcb lp = %d, rp = %d, lip =%x, rip = %x, ssid = %d",
	      npcb->local_port,npcb->remote_port,
	      npcb->local_ip,npcb->remote_ip,npcb->ssid);
...
tcp_process(struct tcp_pcb *pcb)
{
  ...
  case SYN_RCVD:
	...
	DEBUG_TCPIN kprintf(KR_OSTREAM,"TCP connection established %d -> %d", 
  ...
  case ESTABLISHED:
	...
	if(!constructed) {
	  constructor_request(KR_CONSTREAM,KR_BANK,KR_SCHED,
			      KR_VOID,KR_CONSTREAM);
	}
	constructed = 1;
	
	ints = (void *)(2*Qstart + start_addr);
  ...
  case FIN_WAIT_1:
	...
	DEBUG_TCPIN 
	  kprintf(KR_OSTREAM,"1 TCP connection closed %d -> %d.", 
		  inseg.tcphdr->src, inseg.tcphdr->dest);
	tcp_ack_now(pcb);
	tcp_pcb_purge(pcb);
  ...
  case FIN_WAIT_2:
    tcp_receive(pcb);
    if (flags & TCP_FIN) {
      DEBUG_TCPIN kprintf(KR_OSTREAM,"2 TCP connection closed %d -> %d", 
			  inseg.tcphdr->src, inseg.tcphdr->dest);
      tcp_ack_now(pcb);
      tcp_pcb_purge(pcb);
      TCP_RMV(&tcp_active_pcbs, pcb);
  ...
  case CLOSING:
    tcp_receive(pcb);
    if (flags & TCP_ACK && ackno == pcb->snd_nxt) {
      DEBUG_TCPIN  kprintf(KR_OSTREAM,"3 TCP connection closed %d -> %d",
			   inseg.tcphdr->src, inseg.tcphdr->dest);
      tcp_ack_now(pcb);
      tcp_pcb_purge(pcb);
      TCP_RMV(&tcp_active_pcbs, pcb);
  ...
  case LAST_ACK:
    tcp_receive(pcb);
    if (flags & TCP_ACK && ackno == pcb->snd_nxt) {
      DEBUG_TCPIN kprintf(KR_OSTREAM,"4 TCP connection closed %d -> %d", 
			  inseg.tcphdr->src, inseg.tcphdr->dest);
      pcb->state = CLOSED;
 ...
 * or pcb->ooseq). If the segment is buffered, the pstore is referenced by 
 * pstore_ref so that it will not be freed until it has been removed from
...
tcp_receive(struct tcp_pcb *pcb)
{
  ...
  if (flags & TCP_ACK) {
	...
	DEBUG_TCPIN
	  kprintf(KR_OSTREAM,"tcp_receive: queuelen1 %d ..",pcb->snd_queuelen);
	pcb->snd_queuelen -= pstore_clen(next->p,pcb->ssid);
	tcp_seg_free(next,pcb->ssid);
    ...
    DEBUG_TCPIN 
      kprintf(KR_OSTREAM,"tcp_receive: pcb->rttest %d rtseq %d ackno %d",
    ...
    if (pcb->rttest && TCP_SEQ_LT(pcb->rtseq, ackno)) {
      ...
      DEBUG_TCPIN
	kprintf(KR_OSTREAM,"tcp_receive: experienced rtt %d ticks(%d msec)",
	...
	m = -m;
      ...
      DEBUG_TCPIN 
	kprintf(KR_OSTREAM,"tcp_receive: RTO %d (%d miliseconds)",
  ...
  if (tcplen > 0) {
    ...
    */
	...
	/* Trimming the first edge is done by pushing the payload
	   pointer in the pstore downwards. This is somewhat tricky since
	   we do not want to discard the full contents of the pstore up to
	   the new starting point of the data since we have to keep the
	   TCP header which is present in the first pstore in the chain.
	   
	   What is done is really quite a nasty hack: the first pstore in
	   the pstore chain is pointed to by inseg.p. Since we need to be
	   able to deallocate the whole pstore, we cannot change this
	   inseg.p pointer to point to any of the later pstores in the
	   chain. Instead, we point the ->payload pointer in the first
	   pstore to data in one of the later pstores. We also set the
	   inseg.data pointer to point to the right place. This way, the
	   ->p pointer will still point to the first pstore, but the
	   ->p->payload pointer will point to data in another pstore.
	   
	   After we are done with adjusting the pstore pointers we must
	...
	if (inseg.p->len < off) {
	  ...
	  while (p->len < off) {
	    ...
	    p->len = 0;
	    p = PSTORE_NEXT(p,pcb->ssid);
	  }
	  pstore_header(p, -off,pcb->ssid);
	} else {
	  pstore_header(inseg.p, -off,pcb->ssid);
	}
	inseg.dataptr = PSTORE_PAYLOAD(inseg.p,pcb->ssid);
	...
	if (pcb->ooseq != NULL &&
	  ...
	  pstore_realloc(inseg.p, inseg.len,pcb->ssid);
	}
	...
	/* If there is data in the segment, we make preparations to
	   ...
	   is used for holding the pstore that goes to the
	   application. The code for reassembling out-of-sequence data
	   chains its data on this pstore as well.
	...
	if (inseg.p->tot_len > 0) {
	  recv_data = inseg.p;
	  /* Since this pstore now is the responsibility of the
	...
	while (pcb->ooseq != NULL &&
	  ...
	  if (cseg->p->tot_len > 0) {
	    /* Chain this pstore onto the pstore that we will pass to
	       the application. */
	    if (recv_data) {
	      pstore_chain(recv_data, cseg->p,pcb->ssid);
	      pstore_free(cseg->p,pcb->ssid);
	    } else {
	      recv_data = cseg->p;
	    }
	    cseg->p = NULL;
	...
	if (pcb->ooseq == NULL) {
	  pcb->ooseq = tcp_seg_copy(&inseg,pcb->ssid);
	} else {
	  ...
	  for(next = pcb->ooseq; next != NULL; next = next->next) {
		...
		/* The incoming segment is larger than the old
		   ...
		   one. */
		cseg = tcp_seg_copy(&inseg,pcb->ssid);
		if (cseg != NULL) {
		  cseg->next = next->next;
		  if (prev != NULL) {
		    prev->next = cseg;
		...
		if (cseg != NULL) {		 		  
		  ...
		  if (TCP_SEQ_GT(prev->tcphdr->seqno + prev->len, seqno)) {
		    ...
		    pstore_realloc(prev->p, prev->len,pcb->ssid);
		  }
		}
		break;
	      }
		...
		next->next = tcp_seg_copy(&inseg,pcb->ssid);
		if (next->next != NULL) {		
		  if (TCP_SEQ_GT(next->tcphdr->seqno + next->len, seqno)) {
		    /* We need to trim the last segment. */
		    next->len = seqno - next->tcphdr->seqno;
		    pstore_realloc(next->p, next->len,pcb->ssid);
		  }
		}
		break;
	      }
	...
	TCP_SEQ_GEQ(seqno, pcb->rcv_nxt + pcb->rcv_wnd)) {
      DEBUG_TCPIN kprintf(KR_OSTREAM,"Acking now");
      tcp_ack_now(pcb);
    }      
    DEBUG_TCPIN kprintf(KR_OSTREAM,"Just exiting");
  }   
}
