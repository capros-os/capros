...
uint32_t lance_interrupt();
uint32_t lance_init();
uint32_t lance_open();
uint32_t lance_close();
uint32_t lance_init_ring();
uint32_t lance_start_xmit(struct pstore *, int ssid);
uint32_t lance_rx();
uint32_t lance_probe(struct pci_dev_data *, struct netif *);
uint32_t lance_alloc_irq();
