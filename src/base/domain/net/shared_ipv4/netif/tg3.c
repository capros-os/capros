...
#define XMIT_START      0x00017000u /* start of the xmit area */
...
extern int stack_mapped;            /* Is the stack mapped already ??*/
...
patch_addrspace_tg3()
{
  ...
  next_slot++;
  node_swap(KR_SCRATCH, next_slot, KR_REGS, KR_VOID);
}
...
tg3_rx()
{
  ...
  if(sw_idx != hw_idx) {
    ...
    if(stack_mapped) demux_pkt((char *)base,len);
...
uint32_t
tg3_start_xmit(struct pstore *p,int ssid)
{
  uint32_t entry, base_flags;
  struct tg3 *tp = &TP;
  uint32_t totlen = 0;
  struct pstore *q;
  unsigned char *tbuf = (void *)(DMA_START + XMIT_START);
  int32_t nextoff;
  char *s;
  ...
  tg3_set_txd(tp, entry, PHYSADDR + XMIT_START,totlen, base_flags,1);
  entry = NEXT_TX(entry);
...
uint32_t
altima_probe(struct pci_dev_data *net_device,struct netif *net_if)
{
  result_t result;
  struct tg3 *tp = &TP;
  int i;
  ...
  net_if->interrupt = tg3_interrupt;
  net_if->start_xmit = tg3_start_xmit;
  net_if->rx = tg3_rx;
  net_if->enable_ints = tg3_enable_int;
  net_if->disable_ints = tg3_disable_int;
  return RC_OK;
}
