...
udp_new(void) {
  ...
  DEBUGUDP kprintf(KR_OSTREAM,"udp new successful at %08x",&pcb[0]);
  return pcb;
}

/* Send data using udp.*/
uint32_t
udp_send(struct udp_pcb *pcb, struct pstore *p)
{
  struct udp_hdr *udphdr;
  struct ip_addr *src_ip=NULL;
  uint32_t err;
  struct pstore *q=NULL; /* q will be sent down the stack*/
  
  DEBUGUDP kprintf(KR_OSTREAM,"udp_send");
  ...
  if(pstore_header(p, UDP_HLEN,pcb->ssid)) {
    kprintf(KR_OSTREAM,"Allocating excess space");
    q = pstore_alloc(PSTORE_IP, UDP_HLEN,p->sector,pcb->ssid);
    if(q == NULL) return RC_NetSys_PbufsExhausted;
    pstore_chain(q, p,pcb->ssid);
  }else {
    q = p;
  }
  
  udphdr = PSTORE_PAYLOAD(q,pcb->ssid);
  ...
  if(pcb->flags & UDP_FLAGS_UDPLITE) {
    ...
    /* calculate checksum */
    udphdr->chksum = inet_chksum_pseudo(p,pcb->ssid,src_ip, &(pcb->remote_ip),
                                        IP_PROTO_UDP, pcb->chksum_len);
    
    /* Avoid collision if chksum is 0x0000 (no chksum) */
    if(udphdr->chksum == 0x0000) udphdr->chksum = 0xffff;
    err = ip_output(q,pcb->ssid,src_ip,&pcb->remote_ip,UDP_TTL, 
		    IP_PROTO_UDPLITE); 
  } else {
    ...
    err = ip_output(q,pcb->ssid,src_ip,&pcb->remote_ip,UDP_TTL,IP_PROTO_UDP);
  }
  return err;
}
...
void
udp_input(struct pstore *p,int ssid)
{
  struct udp_hdr *udphdr;
  struct udp_pcb *pcb;
  struct ip_hdr *iphdr;
  ...
  if (pstore_header(p, -((int16_t)(UDP_HLEN + IPH_HL(iphdr) * 4)),ssid)) {
    /* drop short packets */
    kprintf(KR_OSTREAM,"Dropping short packet");
    pstore_free(p,ssid);
    return;
  }

  udphdr = (struct udp_hdr *)((uint8_t *)PSTORE_PAYLOAD(p,ssid) - UDP_HLEN);
...
udp_recv(struct udp_pcb *pcb,
         void (* recv)(void *arg, struct udp_pcb *upcb, struct pstore *p,
